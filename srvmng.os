...
Функция main()
	...
	Для чЭлемент = 0 По мАргументы.Количество() - 1 Цикл
		...
		Если НРег(мАргументы[чЭлемент]) = "-v" ИЛИ НРег(мАргументы[чЭлемент]) = "--verbosity"Тогда
			бВыводВТерминал = Истина;
		КонецЕсли;
	КонецЦикла;
	
	НастроитьЛогирование(бВыводВТерминал);
...
Процедура НастроитьЛогирование(Знач бВыводВТерминал)
	Перем ФайлЛога, сПолныйПутьКЛогу, КонсольЛога;

	м_оЛог = Логирование.ПолучитьЛог("srvmng.app.message");
	м_оЛог.УстановитьУровень(УровниЛога.Информация);

	сПолныйПутьКЛогу = ОбъединитьПути(ИнформацияОСценарии.Каталог, "srvmng.log");
	ФайлЛога = Новый ВыводЛогаВФайл();
	ФайлЛога.ОткрытьФайл(сПолныйПутьКЛогу);
	м_оЛог.ДобавитьСпособВывода(ФайлЛога);
	Если бВыводВТерминал Тогда
		КонсольЛога = Новый ВыводЛогаВКонсоль();
		м_оЛог.ДобавитьСпособВывода(КонсольЛога);
	КонецЕсли;
